...
// 안 찍힌(미투자) 상태의 노드 아이콘 — API로는 구할 수 없는 미투자 노드 아이콘을 사용자가 제공한
// 이미지(실제 인게임 잠김 화면 스크린샷 2장 → 4티어, 이후 컬러 아이콘이 깔끔한 참고표 1장 → 위 4티어를
// 더 선명한 버전으로 교체 + 4티어(회심 그룹, 이 참고표엔 누락됨)는 스크린샷 버전 유지)에서 칸 위치별로
// 잘라 `loa-cp/icons/evolution/t{티어}c{열}.png`로 저장해 둠(30개 전부 커버). 참고표 아이콘은 색이
// 선명한("찍힌 것처럼 보이는") 원본이라, 미투자 표시는 UI에서 CSS grayscale 필터로 흐리게 처리한다.
